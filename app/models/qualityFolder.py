...
class QualityFolder(db.Model):
	...
	owned_attributes = relationship("Attribute", backref="quality_folder", cascade="all, delete-orphan", single_parent=True)
	########################
	risk_level = db.Column(db.Integer)
	risk_occurrence = db.Column(db.Integer)
	closing_date = db.Column(db.DateTime)
	#actions relationships##
	owned_corrective_actions = db.relationship('CorrectiveAction', backref="quality_folder", cascade="all, delete-orphan", lazy="dynamic") 
	owned_preventive_actions = db.relationship('PreventiveAction', backref="quality_folder", cascade="all, delete-orphan", lazy="dynamic") 
	owned_curative_actions = db.relationship('CurativeAction', backref="quality_folder", cascade="all, delete-orphan", lazy="dynamic") 
	########################
	def __repr__(self):
